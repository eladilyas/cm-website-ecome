      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1.05fr_1.2fr] gap-0">
        ...
        <div className="p-8 md:p-10 lg:p-12 flex flex-col justify-between gap-10">
          <div>
            ...
            <h2 className="mt-4 text-h2 font-semibold tracking-[-0.018em] leading-[1.1] text-ink">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Link
              href="/shop"
              className="ease-brand inline-flex items-center justify-center h-11 px-5 rounded-full bg-ink text-paper text-[14px] font-medium hover:bg-ink-soft transition-colors duration-300"
            >
              {t("browseCta")}
            ...
            <Link
              ...
              className="ease-brand inline-flex items-center text-[14px] font-medium text-ink hover:text-brand transition-colors duration-200"
            >
              {t("talkCta")}
            </Link>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            {TILES.map((tile) => (
              <Link
                key={tile.slug}
                href={`/shop/${tile.slug}`}
                className="ease-brand group relative aspect-square rounded-2xl bg-paper ring-1 ring-hairline overflow-hidden transition-all duration-400 hover:ring-hairline-strong hover:-translate-y-0.5"
              >
                <div className="absolute inset-0 flex items-center justify-center p-5 md:p-6">
                  <Image
                    ...
                    className="ease-brand object-contain p-6 transition-transform duration-500 group-hover:scale-[1.04]"
                  />
                </div>
