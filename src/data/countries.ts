// All ISO 3166-1 countries + dial codes. Used by the PhoneField's country
// selector. Flag emoji is derived at runtime from the ISO code (each ISO
// letter maps to a Regional Indicator Symbol), so we don't store flags in
// the data and bundle weight stays tiny.
...
export const COUNTRIES: Country[] = [
  ...
  // ── Rest of the world, alphabetical by name ─────────────────────────
...
/** Compute the flag emoji for an ISO 3166-1 alpha-2 country code. */
export function flagEmoji(iso: string): string {
  if (!iso || iso.length !== 2) return "";
  const base = 127397; // 'A' is U+1F1E6; 'A'.charCodeAt(0) = 65; offset = 127462 - 65
  return String.fromCodePoint(
    ...iso.toUpperCase().split("").map((c) => c.charCodeAt(0) + base)
  );
}
...
/** Default country for the form. */
export const DEFAULT_COUNTRY = COUNTRIES[0]; // Morocco

/** Diacritic-insensitive, lowercased, trimmed. */
function normalize(s: string): string {
  return s.normalize("NFD").replace(/[̀-ͯ]/g, "").toLowerCase().trim();
}

/** Filter the country list by a free-text query. Matches name OR dial code. */
export function searchCountries(query: string, limit = 12): Country[] {
  ...
  for (const c of COUNTRIES) {
    ...
    if (n.startsWith(q) || c.dial.startsWith(q)) starts.push(c);
